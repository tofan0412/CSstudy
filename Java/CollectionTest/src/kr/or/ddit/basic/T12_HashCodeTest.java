...
public class T12_HashCodeTest {
	public static void main(String[] args) {
		/*
		 * 해시함수(hash function)는 임의의 길이의 데이터를 고정된 길이의 데이터로 매칭하는 
		 * 함수이다. 해시함수에 의해 얻어지는 값은 해시값, 해시코드, 해시체크섬 또는 간단하게
		 * 해시라고도 한다.
		 * HashSet, HashMap, Hashtable와 같은 객체들을 사용할 경우 객체가 서로
		 * 같은지를 비교하기 위해 equals() 메서드와 hashCode() 메서드를 호출한다.
		 ...
		 * 5. hashCode()는 기본적으로 Heap 메모리에 있는 각 객체에 대한 메모리 주소 매핑 정보를 기반으로 
		 * 한 정수값을 반환한다.
		...
		System.out.println("add(p3) 이후 데이터");
		for (Person p : set) {
			System.out.println(p.getId() + ":" + p.getName());
		}
		...
		System.out.println("remove(p2) 이후 데이터");
		for (Person p : set) {
			System.out.println(p.getId() + ":" + p.getName());
		}
	}
