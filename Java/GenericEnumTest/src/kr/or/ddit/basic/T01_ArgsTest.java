...
public class T01_ArgsTest {
	...
	// 가변형 인수를 이용한 메서드
	public int sumArg(int...data) { 	//int값이 들어올 건데, 몇개가 들어오는지는 알 수 없다. 내부적으로는 배열처럼 처리된다 .`
		int sum = 0;
		for (int i = 0 ; i < data.length; i++) {
			sum += data[i];
		}
	...
	// int...data, String name와 같이 작성할 수 없다. 
	public String sumArg2(String name, int...data) {	//String 값은 고정적으로 하나만 들어온다. 
		int sum = 0; 
		for (int i = 0 ; i < data.length ; i++) {
			sum += data[i];
		}
