...
/*
 * yield() 메서드에 관하여 ->
 * 1. 현재 실행 대기중인 동등한 우선순위의 다른 쓰레드에게 실행 기회를 제공한다. 
 * 2. 현재 실행중인 스레드의 상태를 Runnable상태로 바꾼다. 
 * (Waiting이나 Blocked 상태로 바뀌지 않는다.)
 * yield() 메서드를 실행한다고 해서 현재 실행중인 스레드가 곧바로 runnable 상태로
 * 전이된다고 확신할 수 없다. 
 * 
 */
...
class ThreadYield extends Thread {
	...
	public ThreadYield(String name) {
		// Thread에는 기본적으로 name 속성이 있고, Thread 생성자 중에서는
	...
	public void run() {
		while (!stop) { // stop 변수값이 true이면 반복을 멈춘다.
			if (work) {
				// 작업하는 영역
				// getName() => 현재 실행중인 쓰레드의 name값을 반환한다.
